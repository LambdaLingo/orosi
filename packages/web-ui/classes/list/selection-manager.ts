...
import { compareNodeOrder, getChildNodes, getFirstItem } from "utilities";
...
export class SelectionManager implements MultipleSelectionManager {
  ...
  extendSelection(toKey: Key): void {
    ...
    if (this.state.selectedKeys === "all") {
      ...
      const anchorKey = selectedKeys.anchorKey || toKey;
      ...
      for (const key of this.getKeyRange(toKey, anchorKey)) {
  ...
  replaceSelection(key: Key): void {
    ...
    const selection = this.canSelectItem(key)
  ...
  isLink(key: Key): boolean {
    return Boolean(this.collection.getItem(key)?.props?.href);
  }
}
